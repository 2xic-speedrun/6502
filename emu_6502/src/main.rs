...
fn main() {
    ...
    if (isDebugMode) {
        ...
        .create(true)
        .write(true)
        .open("/home/brage/Desktop/6502/outputs/impl.txt")
        .unwrap();
    //   file.close();

        for x in 0..500 {
            let opcode_number = machine.get_opcode();
            if let Err(e) = writeln!(file, "pc: {:#04x} sp: {:#04x} opcode: {:#04x}", machine.register.pc, machine.register.sp, opcode_number) {
                eprintln!("Couldn't write to file: {}", e);
            }
            if let Err(e) = writeln!(file, "N: {}", (machine.register.n() as i8)) {
                eprintln!("Couldn't write to file: {}", e);
            }
            if let Err(e) = writeln!(file, "y: {}, x: {}, ac: {}\n", machine.register.y, machine.register.x, machine.register.ac) {
                eprintln!("Couldn't write to file: {}", e);
            }
